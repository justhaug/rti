...
pub struct OracleConfig {
    ...
    /// The game's replay Autosaves folder; new PB replays there are imported
    /// automatically by the server loop and `rti replay watch`.
    pub tm2020_replays_dir: String,
...
impl Default for OracleConfig {
    fn default() -> Self {
        OracleConfig {
            ...
            tm2020_replays_dir: default_replays_dir(),
...
fn default_replays_dir() -> String {
    let home = std::env::var("HOME").unwrap_or_default();
    let p = format!("{home}/.steam/root/steamapps/compatdata/2225070/pfx/drive_c/users/steamuser/Documents/Trackmania/Replays/Autosaves");
    if std::path::Path::new(&p).is_dir() {
        p
    } else {
        String::new()
